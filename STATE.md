...
- Spin-offs + reverse splits are configured in **`config/events.json`** (since 2026-07-21) — `lib/events.ts` imports and re-exports the typed `SPINOFFS`/`REVERSE_SPLITS` constants, and `scripts/digest.swift` reads the same file via `loadEvents()` so the digest pipeline's portfolio math and spin-off framing can't drift from the app's. `config/events.json` is statically imported → must NOT be excluded in `scripts/vercel-ignore-build.sh`. A spin-off child is fetched as if it had a START_DATE of its `effectiveDate` (no backtracked history — value is purely additive from listing day forward). Children are surfaced as first-class holdings/stocks via `SPINOFF_CHILD_TICKERS` + the derived-ownership augmentation in `TICKER_OWNERS`, **without** being added to any user's `tickers` array (that would change `perHoldingDollars` = $100k/N and dilute the user's other picks). Currently populated: **HON → HONA** (Honeywell Aerospace, ratio 0.5, effective 2026-06-29) plus the bundled **HON 1-for-2 reverse split** (`REVERSE_SPLITS`, same date).
...
WindowDigest {
  ...
  narrativeGeneratedAt?: ISO | null  // when the STORY was authored (generatedAt = "numbers as of",
                                     // bumped by every 15-min render). Added 2026-07-21; optional
                                     // both directions for old/new interleave safety.
  anchorMover?: string | null        // game windows: ticker the narrative frames as top mover /
  anchorDrag?: string | null         // biggest drag — powers the acting sign guard (CLAUDE.md §8.6)
}
...
|---|---|---|---|---|
| `fast` | `cron-update.sh` every 15 min, after the price fetch | no | normally none; ≤ `DIGEST_REGEN_MAX` (default 2) per game window per day when the sign guard trips | game `1D / 1W / 1M` — re-renders each `digestTemplate` with live pcts from the freshly-written `prices.json`; if the frozen narrative now contradicts the live signs (anchor flip or directional-verb disagreement), regenerates that window on PCC (strict, fm-serve-probed) or rebuilds it deterministically. See CLAUDE.md §8.6 "sign guard ACTS". |
...
- **Theme system.** `globals.css` keeps the dark palette as the default `:root` with two alternates: `:root[data-theme="light"]` (Robinhood-light) and `:root[data-theme="twilight"]` (deep-indigo pre-market/after-hours). `<ThemeController>` mounts in the layout and switches `<html data-theme>` based on `getMarketSessionState()` (re-evaluated every 60s); a QA override `?theme=light|twilight|dark` (persisted to localStorage; `?theme=auto` clears) pins the theme for testing. **Since 2026-07-21 the migration target is semantic tokens** — `--surface-*`/`--ink-*`/`--border-*` variables assigned per theme and exposed as utilities (`bg-card`, `text-ink-muted`, `border-hairline`, …) via `@theme inline`; new components use these, never with opacity modifiers (mint a new token in all three theme blocks instead), and every new token's utility joins the theme-transition selector group. App chrome (layout, TabBar, HeaderBack, Footer, Sheet, InstallHint, MarketStateBadge) is migrated; un-migrated components still ride the legacy per-utility overrides, where **each opacity variant is a separate override** — `.bg-zinc-900\/60` is not covered by `.bg-zinc-900` (this is how the "What's new" button broke). `npm run check-theme` enforces legacy coverage in CI + pre-push; see CLAUDE.md §5.6.
